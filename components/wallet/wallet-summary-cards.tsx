...
export function WalletSummaryCards() {
  const { walletOverview, isLoading, fetchWalletOverview } = useWalletStore();
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold">
            ${bridgeAccount.balance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </div>
          <p className="text-xs text-muted-foreground mt-1">
            Available: ${bridgeAccount.availableBalance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </p>
          <div className="flex items-center mt-2">
            <span className={`text-xs flex items-center ${bridgeAccount.status === 'active' ? 'text-green-600' : 'text-yellow-600'}`}>
              <span className={`h-2 w-2 rounded-full mr-1 ${bridgeAccount.status === 'active' ? 'bg-green-600' : 'bg-yellow-600'}`}></span>
              {bridgeAccount.status.charAt(0).toUpperCase() + bridgeAccount.status.slice(1)}
            </span>
          </div>
        </CardContent>
      </Card>
